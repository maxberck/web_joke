...
/** Worth fictif et humoristique, jamais une vraie estimation financière. */
export function computeWorth(
  ...
  const financialFactor = Math.max(0, Math.min(100, financialAbility)) / 100;
  ...
  if (!Number.isFinite(worth)) throw new Error("Worth calculé invalide");
