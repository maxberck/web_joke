...
export const matchBaselines: MatchBaselines = {
  careers: { ...matchBaselinesRaw.careers, ...matchBaselinesExtraRaw.careers }, classes: { ...matchBaselinesRaw.classes, ...matchBaselinesExtraRaw.classes },
  powers: { ...matchBaselinesRaw.powers, ...matchBaselinesExtraRaw.powers }, weaknesses: { ...matchBaselinesRaw.weaknesses, ...matchBaselinesExtraRaw.weaknesses },
  abilities: { ...matchBaselinesRaw.abilities, ...matchBaselinesExtraRaw.abilities }, workStyles: { ...matchBaselinesRaw.workStyles, ...matchBaselinesExtraRaw.workStyles }, animals: { ...matchBaselinesRaw.animals, ...matchBaselinesExtraRaw.animals },
};
...
export function assertRarityDistributionIsValid(table: RarityBucket[]): void { if (table.length === 0) throw new Error("Rarity: table vide"); let previousScore = -Infinity; let previousOneInX = 0; for (const [index, bucket] of table.entries()) { if (!Number.isFinite(bucket.minScore) || !Number.isFinite(bucket.oneInX) || bucket.oneInX < 1) throw new Error(`Rarity[${index}]: valeur invalide`); if (bucket.minScore < previousScore || bucket.oneInX < previousOneInX) throw new Error(`Rarity[${index}]: table non monotone`); previousScore = bucket.minScore; previousOneInX = bucket.oneInX; } }
assertContentPackIsValid(contentPack); assertMatchBaselinesAreValid(matchBaselines); assertRarityDistributionIsValid(rarityDistribution);
